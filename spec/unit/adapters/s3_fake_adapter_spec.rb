require 'zlib'
require 'register_files_combiner/adapters/s3_fake_adapter'

RSpec.describe RegisterFilesCombiner::Adapters::S3FakeAdapter do
  ...
  describe '#download_from_s3' do
    ...
    context 'when s3_path does not match an existing file in bucket' do
      ...
      it 'raises a S3FakeAdapter::Errors::NoSuchKey error' do
        ...
        end.to raise_error(RegisterFilesCombiner::Adapters::S3FakeAdapter::Errors::NoSuchKey)
      end
    end
  ...
  describe '#list_objects' do
    context 'when prefix matches some keys' do
      it 'returns matching keys' do
        keys = subject.list_objects(s3_bucket: s3_bucket, s3_prefix: 'existing/path/t')
        expect(keys).to eq [existing_s3_path]
      end
    end

    context 'when prefix does not match keys' do
      it 'returns an empty array' do
        keys = subject.list_objects(s3_bucket: s3_bucket, s3_prefix: 'existing/path/a')
