require 'register_files_combiner/step_processing/mappers/interest_parser'

RSpec.describe RegisterFilesCombiner::StepProcessing::Mappers::InterestParser do
  subject { described_class.new(error_adapter: error_adapter) }

  let(:error_adapter) { double 'error_adapter' }

  describe '#call' do
    context 'when given a Hash' do
      let(:interest_type) { double 'interest_type' }
      let(:share_min) { 50 }
      let(:share_max) { 50 }
      ...
      context 'with share_min equal to share_min' do
        it 'returns mapped interest' do
          expect(subject.call(interest)).to eq({
            type: interest_type,
            share: {
              exact:   share_min,
              minimum: share_min,
      ...
      context 'with ownership-of-shares-25-to-50-percent*' do
        it 'returns correct interest' do
          [
            ...
            expect(subject.call(value)).to eq({
              type: 'shareholding',
              details: value,
    ...
    context 'when given an unknown type' do
      ...
      it 'raises an error' do
        expect { subject.call(interest) }.to raise_error(described_class::UnexpectedInterestTypeError)
      end
    end
  end
end
